...
def calculateBfield(contours, DSV, resolution, coilRad, direction):
    ...
    error = calculateError(points, bField, direction, gradCurrent)
    return [xSphere*1e3, ySphere*1e3, zSphere*1e3], np.reshape(bField, (np.size(theta), np.size(phi))), error

def calculateError(coords, bField, direction, current):
    ...
    fitResults = np.polyfit(coordAxis, bField, 1)

    efficiency = fitResults[0]*1e3 / current
    
    coordAxis[np.abs(coordAxis) < 0.01*np.ptp(coordAxis)] = 'NaN'

    print("Simulated efficiency: %.4f mT/m/A"%(efficiency))
        
    return np.nanmax((np.abs(bField - fitResults[0]*coordAxis))/(fitResults[0]*coordAxis))
    
def exportWires(contours, coilRad, direction, conjoined):
    
    wireNum = 0                
    gapSize = 11 #gap in which the sections are joined
