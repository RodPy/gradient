...
def SoSXgradient(modes_N = 8, modes_M = 4, radius = 150, length = 450, numContours = 15, MaxLinError = 20, DSV=200, gradDirection='X' ):
    ...
    Nsamp = 100
    outputResMultiplier = 8 #increase in resolution of the output compared to dcurrent distriubition used for field calculations
    ...
    for idx in tqdm(range(np.size(nVec))):  
    ...
    z = np.linspace(-length/2, length/2, num=Nsamp*outputResMultiplier)
    z = np.vstack(z)
    
    if gradDirection == 'X':
        phi = np.linspace(0.5*np.pi, 2.5*np.pi, num = Nsamp*outputResMultiplier)
    elif gradDirection == 'Y':
        phi = np.linspace(0, 2*np.pi, num = Nsamp*outputResMultiplier)
    elif gradDirection == 'Z':
        phi = np.linspace(0.25*np.pi, 2.25*np.pi, num = Nsamp*outputResMultiplier)
...
def exportWires(contours, coilRad, direction, conjoined):
    ...
    joinerSteps     = 36
    gapSize         = 12 #gap in which the sections are joined
    gapRotation     = -0.0979   # WT 20220125: angle (rads) to rotate centre location of gap
    ...
    for idx, wireLevel in enumerate(wireLevels):
        for wire in wireLevel:
            if(conjoined):
                ...
                centerAngle = np.mean(wire[:,0]) + gapRotation   # WT 20220125: added +gapRotation
                centerAngles.append(centerAngle)
                
                if(direction == 0):
                    #mask = (np.abs(wire[:,0] - centerAngle) > gapAngle) | (np.abs(wirePath3D[:,2]) < centerHeight)
                    mask = (np.abs(wire[:,0] - centerAngle) > gapAngle) | (np.abs(wire[:,1])*1e3 > centerHeight)
                else:
                    mask = (np.abs(wire[:,0] - centerAngle) > gapAngle) | (wire[:,1] < 0)
                #make sure the exported wires start at a gap by shifting the array
                shiftedWire = wire
                while mask[0]:
                    mask = np.roll(mask,1)
                    shiftedWire = np.roll(shiftedWire, 1, axis = 0)
                    
                contourDict[str(wireNum)] = np.stack((shiftedWire[mask, 0],shiftedWire[mask, 1]),axis=1)
    ...
    if(not conjoined):
        ...
        angles      = np.linspace(-np.pi, np.pi, joinerSteps+2) #+2 because point start and end point get cut off to avoid having two overlapping points
    
        for idx in range(1,int(wireNum/numCoilSegments)):
            ########################################################################
            startPoint  = joinedContour[str(0)][-1,:]
            endPoint    = contourDict[str(2*idx)][0,:]
            centerAngle = centerAngles[2*idx]

            angleChange = np.min(np.abs([startPoint[0] - centerAngle, endPoint[0] - centerAngle]))
            changeZ     = endPoint[1] - startPoint[1]
            
            angleToPos  = centerAngle + np.round(np.cos(centerAngle))*angleChange*angles[1:-1]/np.pi 
            zPos        = startPoint[1] + changeZ*(np.tanh(angles[1:-1])+1)/2
            
            joinedContour[str(0)] = np.append(joinedContour[str(0)], np.stack((angleToPos,zPos), axis = -1), axis = 0)
            joinedContour[str(0)] = np.append(joinedContour[str(0)], contourDict[str(2*idx)], axis = 0)
            ########################################################################
            startPoint  = joinedContour[str(1)][-1,:]
            endPoint    = contourDict[str(2*idx+1)][0,:]
            centerAngle = centerAngles[2*idx + 1]
            ...
            joinedContour[str(1)] = np.append(joinedContour[str(1)], np.stack((angleToPos,zPos), axis = -1), axis = 0)
            joinedContour[str(1)] = np.append(joinedContour[str(1)], contourDict[str(2*idx + 1)], axis = 0)
            ########################################################################
            startPoint  = joinedContour[str(2)][-1,:]
            endPoint    = contourDict[str(int(2*wireNum/numCoilSegments) + idx*2 )][0,:]
            centerAngle = centerAngles[int(2*wireNum/numCoilSegments) + idx*2]
            ...
            joinedContour[str(2)] = np.append(joinedContour[str(2)], np.stack((angleToPos,zPos), axis = -1), axis = 0)
            joinedContour[str(2)] = np.append(joinedContour[str(2)], contourDict[str(int(2*wireNum/numCoilSegments) + idx*2 )], axis = 0)
            ########################################################################
            startPoint  = joinedContour[str(3)][-1,:]
            endPoint    = contourDict[str(int(2*wireNum/numCoilSegments) + idx*2 +1)][0,:]
            centerAngle = centerAngles[int(2*wireNum/numCoilSegments) + idx*2 +1]
            ...
            joinedContour[str(3)] = np.append(joinedContour[str(3)], np.stack((angleToPos,zPos), axis = -1), axis = 0)
            joinedContour[str(3)] = np.append(joinedContour[str(3)], contourDict[str(int(2*wireNum/numCoilSegments) + idx*2 +1)], axis = 0)
          
        #convert wires to 3D
        joinedContour[str(0)] = np.stack((np.cos(joinedContour[str(0)][:,0])*coilRad,np.sin(joinedContour[str(0)][:,0])*coilRad,joinedContour[str(0)][:,1]*1e3),axis=1) 
        joinedContour[str(1)] = np.stack((np.cos(joinedContour[str(1)][:,0])*coilRad,np.sin(joinedContour[str(1)][:,0])*coilRad,joinedContour[str(1)][:,1]*1e3),axis=1) 
        joinedContour[str(2)] = np.stack((np.cos(joinedContour[str(2)][:,0])*coilRad,np.sin(joinedContour[str(2)][:,0])*coilRad,joinedContour[str(2)][:,1]*1e3),axis=1) 
        joinedContour[str(3)] = np.stack((np.cos(joinedContour[str(3)][:,0])*coilRad,np.sin(joinedContour[str(3)][:,0])*coilRad,joinedContour[str(3)][:,1]*1e3),axis=1) 
